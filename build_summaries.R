...
# the NTs were covered by 5 or more bases.

d <- bind_rows(lapply(1:nrow(representativeSampleSummary_90), function(x){
    x <- representativeSampleSummary_90[x,]
    
    if(x$type == 'composite'){
      f <- file.path('summaries/VSPdata', paste0(x$exp, '.composite.RData'))
    } else {
      f <- file.path('summaries/VSPdata', paste0(x$exp, '.experiment.RData'))
    }
    
    if(! file.exists(f)) stop(paste(f, 'does not exist'))
    load(f)
    
    if(nrow(opt$variantTable) > 0){
      o <- subset(samples, VSP == str_extract(f, 'VSP\\d+'))
      if(nrow(o) != 1) stop(paste0('VSP lookup error for VSP id "', str_extract(f, 'VSP\\d+'), '"'))
      
      opt$variantTable$patient <- o$patient_id
      opt$variantTable$sample  <- o$sample_type
      opt$variantTable$date    <- o$sample_date
      return(opt$variantTable)
    } else {
      return(data.frame())
    }
}))
...
invisible(lapply(split(d, d$patient), function(x){
  positions <- unique(subset(x, percentAlt > 0.5 & ! ALT == 'e')$POS)
  x <- subset(x, POS %in% positions)
  
  x <- bind_rows(lapply(split(x, paste(x$sample, x$date)), function(a){
    m <- data.frame(CHROM = 'genome', POS = positions, REF = 'X', ALT = 'e', QUAL = 225, percentAlt = 1,
                    reads = 1, genes = 'X', type = 'X', patient = a$patient[1], sample = a$sample[1], date = a$date[1])
    
    bind_rows(a, subset(m, POS %in% positions[! positions %in% a$POS]))
  }))
  
  x$ALT <- sub('e', 'Expected', x$ALT)
  x$ALT <- sub('\\?', 'Ins/Del', x$ALT)
  x$ALT <- factor(x$ALT, levels = c('Expected', 'A', 'T', 'C', 'G', 'N', 'Ins/Del'))
  
  x$POS <- factor(x$POS, levels = unique(sort(x$POS)))
  
  p <- ggplot(x, aes(POS, percentAlt, fill = ALT)) + 
       theme_bw() +
       scale_fill_manual(name = 'Base change', drop=FALSE, values = c('gray90', 'green3', 'red2', 'blue2', 'gold2', 'purple', 'black')) +
       geom_col() +
  ...
  ggsave(p, filename = file.path('summaries', 'patientVariantPlots', paste0(unique(x$patient), '.pdf')),
         units = 'in', width = length(unique(x$POS))*0.5, height = length(unique(paste(x$sample, x$date))) * 3)
